...
class SignupRequest(BaseModel):
    ...
    name: str | None = None  # Alternative field name
...
async def register(
    ...
    # Use full_name or name field
    user_name = data.full_name or data.name or data.email.split('@')[0]
    ...
    return success_response(
        data={"user_id": user.id, "email": user.email, "name": user_name},
        message=SUCCESS_MESSAGES["REGISTER"],
        path="/api/v1x/auth/register"
    )
...
async def signup(
    ...
    """Register a new user (v1 alias for compatibility)"""
    return await register(data, request, background_tasks, db)
...
def login(res: Response, request: Request, data: LoginRequest, db: Session = Depends(get_db)):
    ...
    try:
        logger.debug(f"[AUTH] Login attempt from {client_ip}")
        ...
        if not u:
            logger.warning(f"[AUTH] Login failed: user not found - email={data.email}")
        ...
        if not verify_password(data.password, u.password_hash):
            logger.warning(f"[AUTH] Login failed: bad password - user_id={u.id}")
            record_login_attempt(
                db=db,
                user_id=u.id,
                ip_address=client_ip,
        ...
        logger.info(f"[AUTH] Login successful - user_id={u.id}, ip={client_ip}")
        ...
        logger.info("[AUTH] Login complete - cookie set, token issued")
        return success_response(
        ...
        # Catch all other errors with detailed logging
        import traceback
        logger.error(f"[AUTH] ❌ Unexpected error in /auth/login")
        logger.error(f"  IP: {client_ip}")
        logger.error(f"  Email: {data.email if data.email else 'N/A'}")
        ...
        if "locked" in str(e).lower() or "database" in str(e).lower():
            logger.error("[AUTH] Database lock detected - returning 503")
            raise HTTPException(
                status_code=503,
                detail="Database temporarily unavailable. Try again in 30 seconds."
            )
        else:
            logger.error("[AUTH] Generic server error - returning 500")
            raise HTTPException(
                status_code=500,
...
def reset(data: ResetRequest, db: Session = Depends(get_db)):
    try:
        ...
        uid = claims.get("sub")
        if not uid:
            raise HTTPException(status_code=400, detail=ERROR_MESSAGES["INVALID_TOKEN"])
...
def oauth_exchange(provider: str, data: OAuthRequest, response: Response, background_tasks: BackgroundTasks, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return success_response(
            data={"access_token": token, "token_type": "bearer", "user_id": user.id, "email": user.email},
            message=SUCCESS_MESSAGES["LOGIN"],
            path=f"/api/v1x/auth/oauth/{provider}"
        )
    except HTTPException:
        raise
    except Exception as e:
