...
TEST_EMAIL = "test_api_user_" + str(int(time.time())) + "@example.com"
...
class APITester:
    ...
    def setup_auth(self):
        ...
        if r.status_code in [200, 201]:
            ...
            print(f"WARNING: Registration response: {r.status_code} - {r.text[:200]}")
        ...
        if 'token' in r.cookies:
            ...
            print(f"FAIL: No token in cookies. Response: {r.text}")
            return False
    ...
    def run_all_tests(self):
        ...
        if not self.setup_auth():
            print("FAIL: Failed to setup authentication. Exiting.")
            return
    ...
    def print_summary(self):
        ...
        if self.results["errors"]:
            ...
            for error in self.results["errors"]:
                ...
                if 'error' in error:
                    err_text = error['error'][:100] if isinstance(error['error'], str) else str(error['error'])
