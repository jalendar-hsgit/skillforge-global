...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    let target = "";
    if (id) {
      // DELETE or PUT /skills/{id}
      target = `${API_BASE}/api/v1x/resumes/skills/${id}`;
    } else {
      // POST /{resumeId}/skills
      target = `${API_BASE}/api/v1x/resumes/${resumeId}/skills`;
    }
    ...
    res.status(500).json({ detail: e?.message || "skills proxy error" });
  }
}
